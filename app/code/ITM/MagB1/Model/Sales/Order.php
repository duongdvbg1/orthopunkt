...
//    $customer = $objectManager->get('\Magento\Customer\Model\Customer')->load(2);
    ...
    public function saveOrder(\Magento\Sales\Api\Data\OrderInterface $entity)
    {
        ...
        if ($increment_id == "") {
            //$order->setItmCreatedBy($entity->getExtensionAttributes()->getItmCreatedBy());
            $order->setItmCreatedBy("magb1");
        }
        $order->save();
    ...
    public function updateOrderReservedItemsData($order)
    {
        ...
        $select = $connection->select()
            ...
            ->where(
                \Magento\InventoryReservationsApi\Model\ReservationInterface::METADATA . ' LIKE  \'%"object_id":"'.$_id.'"%\' '.
                ' OR '.
                \Magento\InventoryReservationsApi\Model\ReservationInterface::METADATA . ' LIKE  \'%"object_increment_id":"'.$order->getIncrementId().'"%\' '
            )
            //  Mariadb 10.1 which is compatible with 2.3 does not support JSON_EXTRACT
            /*->where(
                "(JSON_UNQUOTE(JSON_EXTRACT(".\Magento\InventoryReservationsApi\Model\ReservationInterface::METADATA.", '$.object_increment_id')) = '". $order->getIncrementId() . "') OR ".
                "(CONVERT(JSON_UNQUOTE(JSON_EXTRACT(".\Magento\InventoryReservationsApi\Model\ReservationInterface::METADATA.", '$.object_id')), UNSIGNED INTEGER) = ". $order->getEntityId() . ")"
            )*/

            ->where(\Magento\InventoryReservationsApi\Model\ReservationInterface::METADATA . ' LIKE  \'%"event_type":"order_placed"%\' '.
                ' OR '.
                \Magento\InventoryReservationsApi\Model\ReservationInterface::METADATA . ' LIKE  \'%"event_type":"magb1_order_updated"%\' '
            )
        ...
        foreach ($order->getItems() as $item) {
            ...
            $sku = $item->getSku();
            $metadata = $this->jsonSerializer->serialize(["event_type" => "magb1_order_updated", "object_type" => "order", "object_id" => $_id, "object_increment_id"=>$order->getIncrementId()]);
    ...
    public function getOrderInfo($increment_id)
    {
        ...
        if ($this->_moduleManager->isEnabled("Amasty_Orderattr")) {
            $oOrderR = $this->_orderRepository->get($order->getId());
            $amastyAttributes = $oOrderR->getExtensionAttributes()->getAmastyOrderAttributes();
            if(!empty($amastyAttributes)) {
                $amasty_extension_attributes = [];
                foreach ($amastyAttributes as $value) {
                    $amasty_extension_attributes[] = ["attribute_code" => $value->getAttributeCode(), "value" => $value->getValue()];
                }
                $extension_attributes["amasty_order_attributes"] = $amasty_extension_attributes;
            }
        }
