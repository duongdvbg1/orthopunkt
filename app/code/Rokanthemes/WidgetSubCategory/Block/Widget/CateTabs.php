...
class CateTabs extends \Magento\Framework\View\Element\Template implements \Magento\Widget\Block\BlockInterface
{
    ...
    public function getSubCategories($category)
    {
		...
		if($this->getData('limit_sub')){
			$limit_sub = $this->getData('limit_sub');
			if(is_numeric($limit_sub)){
				$collection->setPageSize($this->getData('limit_sub'));
			}
		}	
        return $collection;
    }
        ...
        {   $classImage = $this->getData('show_category_image_type');
            if($img = $category->getData($type))
            {
                ...
                if(is_array($trim_pub) && count($trim_pub) > 0){
                    ...
                    $mediaUrl = $this ->_storeManager-> getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA );
                    $url_fixed = $mediaUrl.'catalog/category/thumbnail/'.$path_media; 
                }
    ...
    public function getCountProductCategory($category)
    {
        ...
        $categoryFilter->apply($request);
        $layer->apply();
       return '<span class="count"><span class="bracket">(</span>' .  __('%1 items', $layer->getProductCollection()->getSize()).'<span class="bracket">)</span></span>';
    }
}
?>
