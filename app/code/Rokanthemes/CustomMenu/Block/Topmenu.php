...
class Topmenu extends \Magento\Framework\View\Element\Template
{
            ...
            foreach($children as $child) {
                ...
                if (!$rt_menu_hide_item) {
                    ...
                    if($cat_model->getData('rt_menu_bg_img') != ''){
                         $custom_style = "background-image: url('" .$this->_helper->getBgImageUrl($cat_model). "');";
                    ...
                    if($level == 1 && $rt_menu_icon_img) {
                        $html .= '<div class="menu-thumb-img"><a class="menu-thumb-link" href="'.$this->_categoryHelper->getCategoryUrl($child).'"><img src="' .$this->_helper->getIconimageUrl($cat_model). '" alt="'.$child->getName().'"/></a></div>';
                    }
                    $html .= '<a href="'.$this->_categoryHelper->getCategoryUrl($child).'">';
                    if ($level > 1 && $rt_menu_icon_img)
                        $html .= '<img class="menu-thumb-icon" src="' .$this->_helper->getIconimageUrl($cat_model). '" alt="'.$child->getName().'"/>';
    ...
    public function getCustomMenuHtml()
    {
		...
		if($this->getCustomBlockHtml('before')){
			$html .= '<li class="level0">'.$this->getCustomBlockHtml('before').'</li>';
		}
        foreach($categories as $category) {
            ...
            if(!$rt_menu_hide_item) {
                ...
                $css_fixed = array();
                if($cat_model->getData('rt_menu_bg_img') != ''){
                     $css_fixed[] = "background-image: url('" .$this->_helper->getBgImageUrl($cat_model). "')";
                }
                ...
                if ($rt_menu_icon_img)
                    $html .= '<img class="menu-thumb-icon" src="' . $this->_helper->getIconimageUrl($cat_model) . '" alt="'.$category->getName().'"/>';
                elseif($rt_menu_font_icon)
                    $html .= '<em class="menu-thumb-icon '.$rt_menu_font_icon.'"></em>';
		...
		if($this->getCustomBlockHtml('after')){
			$html .= '<li class="level0">'.$this->getCustomBlockHtml('after').'</li>';
		} 
        return $html;
    }
}
